...
class DiceAdventurePythonEnvRL(Env):
    ...
    def action_masks(self):
        ...
        try:
            ...
            if current_phase == "Player_Pinning":
                ...
                mask[0:4] = True  # left, right, up, down (to control the pin cursor)
                mask[6:] = True  # submit, pinga, pingb, pingc, pingd (to determine which pin to place)
            else:  # Planning phase
            ...
            print(f"[Env {self.env_id}-{self.player}] Error creating action mask: {e}")
    ...
    def step(self, action) -> tuple:
        ...
        """
        # print(f"[Env {self.id}-{self.player}] Stepping with action: {action}")
        if self.train_mode:
    ...
    def _step_train(self, action):
        try:
            ...
            # Check termination immediately after agent's action
            terminated = next_state[0]["currLevel"] != state[0]["currLevel"]
            game_over = next_state[0]["status"] == "GAME_OVER"
            
            # Calculate reward based on agent's action
            reward = self._get_reward(state, next_state)

            # Only process other players if game hasn't terminated
            if not terminated and not game_over:
                ...
                for op in other_players:
                    ...
                    # Check if other players' actions caused termination
                    if next_state[0]["currLevel"] != state[0]["currLevel"]:
                        terminated = True
                        break
                    if next_state[0]["status"] == "GAME_OVER":
                        game_over = True
                        break
            ...
            return next_obs, reward, terminated or game_over, truncated, info
    ...
    def get_observation(self, state: list[dict]) -> np.ndarray:
        ...
        try:
            ...
            for obj in state[1:]:
                ...
                if sight_status == "":
                    # print(f"[Env {self.id}-{self.player}] Sight status is empty for entity_type: {entity_type}, obj_key: {obj_key}")
                    continue
    ...
    def execute_action(self, player: str, game_action: str):
        ...
        try:
            print(f"Executing {game_action} for {player}")
            socket = self._get_socket(player)
            
            if socket is None:
                logging.error(f"Socket is None for player {player}")
                return None
            
            # Execute action
            response = socket.execute_action(game_action)
            print(f"Action execution response: {response}")
            
            # Get new state
            new_state = self.get_state(player)
            if new_state is None:
                logging.error("Got None state after action execution")
            else:
                logging.info(f"New phase: {new_state[0].get('currentPhase')}")
...
s
        ...
        """
        print(f"[Env {self.env_id}-{self.player}] getting state for {player}")
        return _simplify_state(self._get_socket(player).get_state(), player, full_state=self._get_socket(player).get_state("full"))
        ...
        try:
            ...
            for obj in next_state[1:]:
                if (obj.get("objKey", "").startswith("K") and 
                    ...
                    new_shrines.append(obj.get("objKey"))
                    self.reached_shrines.add(obj.get("objKey"))

                    reward += 1
            if len(new_shrines) > 0:
                print(f"[Env {self.env_id}-{self.player}] New shrines: {new_shrines},total reached shrines: {self.reached_shrines}, reward received: {reward}")
            
            # Check level completion
            if state[0]["currLevel"] != next_state[0]["currLevel"]:
                print(f"[Env {self.env_id}-{self.player}] Level completed!")
                reward += 10
                return reward
            ...
            print(f"[Env {self.env_id}-{self.player}] Error calculating reward: {e}")
            return 0
    ...
    def kill_game(self):
        ...
        try:
            if self.game_process:
                ...
                # Kill parent
                parent.kill()
                print(f"[Env {self.env_id}-{self.player}] Game process killed successfully")
        except (psutil.NoSuchProcess, AttributeError) as e:
            print(f"[Env {self.env_id}-{self.player}] Process already terminated or not found: {e}")
        finally:
    ...
    def launch_game(self, game_executable_filepath, port):
        ...
        try:
            ...
            print(f"[Env {self.env_id}-{self.player}] Game launched with PID: {self.game_process.pid}")
            return True
        except Exception as e:
            print(f"[Env {self.env_id}-{self.player}] Error launching game: {e}")
            return False

    
    # --- new functions ---

def _simplify_state(state: dict, player: str, full_state=None) -> list[dict]:
    """
    ...
    return _add_sight_status(scene, player, game_data_obj, full_state)
...
def _launch_game_thread(game_executable_filepath, port):
    ...
    print(f"[Env {self.env_id}-{self.player}] Game launched!!")
    sleep(5)



    
def _add_sight_status(scene: list[dict], player: str, game_data_obj: dict, full_state=None) -> list[dict]:
    """
    ...
    if player_obj is None:
        print(f"player_obj is None for player: {player}")
        print(f"scene: {scene}")
...
def play(self, agents: list[tuple[str, DiceAdventureAgent]]) -> None:
    ...
    for player_name, _ in agents:
        print(f"[Env {self.env_id}-{self.player}] ***Registering agent for: {player_name}***")
        # Register with game
        self._get_socket(player_name).register(player_name)
        print(f"[Env {self.env_id}-{self.player}] ***Agent for: ({player_name}) Registered.***")
